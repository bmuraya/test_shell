...
/**
 *  bring_line_var - assigns the line var for get_line_input
...
/**
 *  get_line_input- Read input from stream
...
ssize_t get_line_input(char **lineptr, size_t *n, FILE *stream)
{
	...
	while (t != '\n')
	{
		...
		if (input >= BUFSIZE)
			buffer = realloc_mem_blc(buffer, input, input + 1);
		buffer[input] = t;
		input++;
	}
	buffer[input] = '\0';
	bring_line_var(lineptr, n, buffer, input);
	retval = input;
	if (i != 0)
		input = 0;
	return (retval);
